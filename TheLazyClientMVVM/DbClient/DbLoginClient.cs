...
namespace TheLazyClientMVVM.DbClient
{
    class DbLoginClient
    {
        ...
        static public bool register(string username, string password, string realName, string email)
        {
                 if (!Com.main.onlineMode) { return false; }
                MySqlConnection c = DbClient.getConnection();
                c.Open();
                MySqlCommand cmd = new MySqlCommand();
                cmd.Connection = c;
                ...
                cmd.Parameters.AddWithValue("_realname", realName);
                cmd.Parameters["_realname"].Direction = ParameterDirection.Input;
        ...
        static public void closeSession(int session_id)
        {
            if (!Com.main.onlineMode) { return; }
            MySqlConnection c = DbClient.getConnection();
            c.Open();
